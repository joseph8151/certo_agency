...
2. `INQUIRY_DEBUG=1` 을 Variable 로 추가하고 재배포한 뒤 다시 `/api/contact` 를 엽니다.
   ...
   `resend.ok` 가 `true` 면 키는 정상입니다. `status` 만 보고 판단하지 마세요.

   | 응답 | 뜻 |
   | --- | --- |
   | `ok: true`, `200` | 키 정상 (Full access) |
   | `ok: true`, `401 restricted_api_key` | **키 정상** — 발송 전용 키라 도메인 조회만 막힌 것 |
   | `ok: false`, `401` | 키가 잘못되었거나 삭제됨 (`re_` 로 시작하는 값 전체를 붙여넣었는지) |
   | `ok: false`, `null` | Worker 에서 `api.resend.com` 으로 나가지 못함 |

   키가 정상인데 메일이 안 오면 폼을 한 번 보내세요. 화면에 실패 사유가 그대로 표시됩니다.
